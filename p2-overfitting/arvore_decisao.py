...
def faz_classificacao(x_treino:pd.DataFrame, y_treino:pd.Series, x_teste:pd.DataFrame, y_teste:pd.Series, min_samples_split:float) -> Tuple[List[float],float]:
    ...
    #calcule a acurácia
    y_comparados = y_predicted == y_teste
    acuracia = np.sum(y_comparados)/len(y_teste)
...
def plot_performance_min_samples(X_treino,y_treino,X_teste,y_teste):
    ...
    #plota o resultado
    plt.plot(arr_min_samples,arr_ac_treino,"b--",label="ac_treino")
    plt.plot(arr_min_samples,arr_ac_teste,"r-",label="ac_teste")
